...
uniform sampler2D LUT_a;
uniform sampler2D LUT_b;
uniform float mixer;

void main() {
	vec4 sample = v_vColour * texture2D( gm_BaseTexture, v_vTexcoord );
	float blue = floor( sample.b*255.0 + 0.5 );
    ...
    gl_FragColor = vec4( mix( texture2D( LUT_a, blue_coord + rg_coord ).rgb, texture2D( LUT_b, blue_coord + rg_coord ).rgb, mixer ), sample.a );
}
